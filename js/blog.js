...
Blog.__name__ = ["Blog"];
Blog.init = function() {
	CodeHighlighter.languages.push(CodeHighlighter.HAXE);
	CodeHighlighter.highlightAll(true);
}
Blog.prototype.__class__ = Blog;
...
if(typeof _CodeHighlighter=='undefined') _CodeHighlighter = {}
...
CodeHighlighter.highlightAll = function(addLineNumbers) {
	...
	while(_g1 < _g) {
		var i = _g1++;
		if(pattern.match(pres[i].className)) elements.push({ element : pres[i], language : pattern.matched(1)});
	}
	var iterate = null;
	iterate = function() {
...
CodeHighlighter.highlightUntil = function(stopRule,code,languageName) {
	...
	tryRule = function(rule) {
		...
		case 0:
			...
			html.add(CodeHighlighter.markup(s,type));
	...
	while(true) {
		...
		while(_g < rules.length) {
			var rule = rules[_g];
			++_g;
			switch( (tryRule(rule))[1] ) {
			case 0:
				return { html : html.b.join(""), rest : code};
			case 1:
				next = true;
				break;
			case 2:
...
CodeHighlighter.lineNumbers = function(code) {
	...
	var html = new StringBuf();
	html.b[html.b.length] = "<div class=\"code-line-numbers\" name=\"code-line-numbers\">" == null?"null":"<div class=\"code-line-numbers\" name=\"code-line-numbers\">";
	var _g = 0;
	while(_g < count) {
		var i = _g++;
		if(i != 0) html.b[html.b.length] = "\n" == null?"null":"\n";
		html.add(i + 1);
	}
	html.b[html.b.length] = "</div>" == null?"null":"</div>";
	return html.b.join("");
}
